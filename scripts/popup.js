...
function setStorage(textarea) {
	...
	for (var i = 0; i < lines.length; i++) {
		...
		try { var dummy = new RegExp(search); } catch (e) {
	...
	chrome.runtime.getBackgroundPage(function(background) {
		background.localStorage['search'] = storage['search'];
		background.localStorage['conceal'] = storage['conceal'];
		background.localStorage['replace'] = storage['replace'];
		background.localStorage['replacements'] = storage['replacements'];
		chrome.tabs.query({}, function(tabs) {
			tabs.forEach(function (tab) {
				chrome.tabs.reload(tab.id);
			});
